...
const { h, render, Component } = __webpack_require__(0);
const Matrix = __webpack_require__(3);
const Solution = __webpack_require__(4);
const { randomMatrix, identityMatrix } = __webpack_require__(1);
...
class App extends Component {
  constructor(props) {
    ...
    this.state = {
      matrices: [identityMatrix(3), randomMatrix(3, 3, 0, 10)]
    };
  }
  ...
  render(props, { matrices }) {
    return h(
      ...
      h(
        'div',
        { 'class': 'matrices' },
        matrices.map((values, i) => [h(Matrix, {
          key: i,
          values: values,
          onChange: this.onChange.bind(this, i) }), i !== matrices.length - 1 ? h('div', { 'class': 'mult' }) : h(
          'div',
          { 'class': 'icon-eq equals' },
          '='
        )]),
        h(Solution, { matrices: matrices })
      )
    );
  }
}
...
const { h, Component } = __webpack_require__(0);
...
const { multiply } = __webpack_require__(1);

function solution({ matrices }) {
  return h(
    'div',
    { 'class': 'matrix solution' },
    multiply(matrices).map((row, i) => h(
      'div',
      { key: i, 'class': `row row-${i}` },
      row.map((val, j) => h(
        'div',
